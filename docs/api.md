...
> **token 携带位置（约定）**：一律放 **`data` 顶层**（`data.adminToken`）—— 业务云函数只认这里。`adminAuth.me` / `adminAuth.changePassword` 两种都读（`params.token || event.adminToken`），所以前端**不必为它记特例**。
...
| `-3` | 该账号**已**设置过密码（`setPassword` 只允许首次） | 提示改用登录 / 修改密码 |
...
| GET | `/users/:uid` | 单个用户完整信息（含 email） | `getByUid`（**网关须传 `flat: true`** 才能拿到扁平 DTO + email；C 端不传则仍是嵌套原文） |
...
>
> **mock 与真实已同口径**（2026-09-18）：`src/api/skills.js` 的 `countUsage` 同样扫这 4 处，并把
> `detail:{userSkills,userRating,teamNeeds,teamMissing}` 挂在每个技能的 `usage` 上（真实删除拦截时回传、
> mock 则写进 message 与 `usage.detail`）。唯一差异：真实有 `MAX_SCAN` 截断保护，mock 数据量固定不实现该分支。
...
| 15 | **`skill_rating` 值类型不一致（脏数据）**：库内曾出现 `{"1":"4"}`（值为**字符串**），部分记录为 number → 前端排序/均值/星级渲染会出错或静默失真 | GET /users（`skill_rating`） | mock 统一 number | ✅ **数据侧已修正（2026-09-19，由用户订正）**：库里已改回数字。前端 `api/users.js` 仍保留 `Number()` 归一作为**防御**（历史数据/手改库无法杜绝），非法值丢弃 |
...
7. ~~`desc` 经 `skill_add` 是否可写入？~~ **已定论（2026-09-18 更新）**：旧 `skill_add` 只接收 `name` 故不可写；**整合后的 `skillApi.add` 已支持 `desc`**（`service.add(name, desc)`，非空即落库），新增时即可填描述，管理端新增弹窗已开放该输入框。
8. ~~AI 慢接口网关超时与计费风险？~~ **已确认**：网关 30s、余额充足；但 25s 上限对 30s 网关**余量仅 5s**，建议放大到 60s 或异步化。
9. **`user.tid_list` 里的悬空引用（数据问题，2026-09-19 澄清）**：有用户的 `tid_list` 里留着 `tid=1`，但该队伍文档**已被手工删除**（用户确认：tid=1 无文档，tid=2~12 仍在）→ 管理端会显示"所属队伍 1 支"却查不到任何队伍。
   **这正是"删除不级联"要防的那种状态**，只是这次来自手工删库（`teamsApi.delete` 本身会清理 `tid_list`）。
   **建议**：① 清理存量（把这些用户的 `tid_list` 中的 `1` 去掉，或按 `teams` 现有 tid 求交集重写）；② 展示侧不猜——队伍列显示 `tid_list.length` 是"引用数"而非"存在的队伍数"，如要更准需先按 `teams` 过滤（管理端 v1 保持原样，避免把一次数据清理变成前端逻辑）。
...
| v0.1.19 | **修掉"部署上去还是模拟数据"的根因**：`vite build` 的默认 mode 是 `production`，而当时只有 `.env`（mock）→ 本地 build 与平台 CI 构建都产出 mock 版。新增 **`.env.production`（真后端）**，口径固定为「开发 = mock，生产构建 = 真后端」；侧栏加"演示数据 / 真实数据"标识便于一眼确认。**第 15 条**改记为"数据侧已修正（用户手工订正），前端保留 `Number()` 归一作防御"；**第 10 节新增第 9 条**：`user.tid_list` 中 `tid=1` 属已删队伍的悬空引用（建议数据侧清理） | `.env`、`.env.production`、`src/layouts/AdminLayout.vue`、`README.md`（新增模式表与"一眼看出线上跑哪种"的判别方式） |
