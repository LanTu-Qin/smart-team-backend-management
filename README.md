...
- **两端已打通**：管理端登录体系（`adminAuth`）+ 真实数据读取（赛事 / 用户 / 队伍 / 技能字典）均已接通线上云开发环境；切换回 `npm run dev` 即为纯 mock 演示版；
- **看板仍依赖聚合接口**：`GET /dashboard/overview` 的分组统计尚未在服务端实现（契约第 8 节第 2 条），当前看板在真后端模式下为空态提示，不做假数据兜底；
- 项目**未引入 TypeScript 与自动化测试**（作品集阶段的取舍）；
- 首页头像统一使用**姓名首字母头像**：用户 `avatar` 是 `cloud://` fileID（临时链接会过期），`teams` 集合**没有头像字段**——不为展示凭空造字段。
