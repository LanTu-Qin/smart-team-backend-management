...
- 页面上可自行确认：登录页页脚显示「演示环境…」、侧栏底部显示 `v1.0.0 · 演示数据`；
  （版本号来自 `package.json`，构建期注入，不写死在界面上）
...
> 侧栏底部（`vX.Y.Z · 演示数据` / `vX.Y.Z · 真实数据`，X.Y.Z = `package.json` 的 version）、登录页是否出现"演示账号"提示。
>
> ⚠️ Vite 的环境变量是**构建期替换**：改完必须重新 build（本地）/ 重新"更新服务"（平台），运行时改无效。
...
- **⚠️ 公开站用真数据 = 需要管理员工号登录**：招聘方/访客打开只会看到登录页，且**绝不能把管理员密码公开**
